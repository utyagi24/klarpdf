...
| **M59.8** Object z-order | **Bring to Front / Forward · Send Backward / to Back** for a selected drawn mark (or group) — reorders the page's annotations list, which is both the paint order in the saved PDF *and* the topmost-wins hit order. Right-click + shortcuts; one undo step. Complements M59.6's geometry hit-testing (which already lets you reach an inner mark) by controlling what paints on top and disambiguating overlapping fills. **No model change** — the list order already *is* the z-order. Ordering vs M59.7 is flexible. | WSL (model+tests) + WSLg | Raise/lower a mark or group; paint + hit order follow; bakes in that order |
| **M59.9** Misc improvements | A holding bucket for the small stuff before the R3 tag — bug fixes found in testing, plus the **text-markup colour palettes**: highlight / underline / strikeout get their own *curated* colours in the **Markup ▾ dropdown** (no extra toolbar slot), separate from the pen & shapes stroke picker, because text markup and freehand drawing are different domains. **Two palettes:** highlight = translucent brights (yellow default · green · blue · pink · orange, its own remembered colour); underline + strikeout = opaque proofing colours (red default · blue · green · black, one shared colour). This moves underline/strikeout **off** the M59.5 stroke picker, narrowing that button to "pen & shapes only", and gives highlight a colour choice for the first time. | WSL + WSLg | Pick a highlight / underline colour from the curated set; it bakes and reopens unchanged |
