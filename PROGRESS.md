...
and for a machine with neither, a single downloaded **`install.py`** that needs nothing but a
supported Python — no clone, no `uv`, no `pipx`, no global `pip`. **`klarpdf` is published to PyPI**
with all 29 dependencies pinned exactly, so every install route resolves the set CI tests and the
weekly audit scans, rather than whatever is newest that day. Publishing is Trusted Publishing (OIDC)
off the `release: published` event, so **no API token exists anywhere** and the manual smoke test
that already gated the GitHub Release gates PyPI too. Underneath, `packaging/` now says which product
each file builds (M133) and the wheel installs **one** top-level name instead of four (M134) —
`model`, `util` and a module called plain `version` would otherwise have collided with anyone else's.
Also shipping: **M137**, the dev lock must be compiled off Windows, and a **pypdf 6.15.0 → 6.17.0**
security bump clearing three crafted-PDF parse-DoS advisories. A **minor** under `RELEASE.md` §3's
SemVer rule: new user-facing capability, no breaking change.

**v0.18.0** — **the MCP / Agent Bridge (M39–M44)**, the roadmap's last
...
- [x] **M144** *(unplanned)* **`httpx2` 2.10.0 → 2.12.0 — three advisories in the bridge's HTTP
  ...
  memory allocation) and two moderate (header injection in multipart uploads; a request carrying
  both `Content-Length` and `Transfer-Encoding`). `httpx2` comes in only through the MCP SDK, so
  **only the bridge is affected; the Windows app installer is not.** None of the three was reachable
  through KlarPDF anyway: the bridge talks over stdio and never makes an HTTP request, which
  `tests/test_mcp_no_qt.py` enforces. The fix moves `httpx2` and its exact-pinned partner
  `httpcore2` together in both locks, then regenerates the `pyproject.toml` pin block, the bundle's
  pyproject and its `uv.lock`. All four locks now audit clean. **Users get it only with a release**:
  the published 0.19.0 wheel pins 2.10.0 exactly.

  Doing it found two gaps, both closed here. Naming `httpx2` alone to pip-compile was a silent no-op,
  because the old `httpcore2` pin held it back; `RELEASE.md` §2 now says what to check. And the
  bundle's `uv.lock` had still said **0.18.0** since the v0.19.0 release, which the drift test could
...
- **The dev lock and the bridge lock pin different `typing-inspection` versions** (0.4.3 vs 0.4.4) —
  ...
  the two locks share at different versions. It is harmless as it stands, because CI's `bridge` job
  installs `requirements-mcp.txt` and so tests the bridge against the version users get. **The
  decision is whether the two locks should be kept in step for shared packages**, as M115 did for
  PyMuPDF through `requirements-core.in`. Doing that for one pydantic helper would mean a constraint
  file or a second recompile on every bump; leaving it means the headless suite runs the bridge on a
  version no user installs. It was left out of M144 so the security fix stayed a two-package diff.

- **The release version is restated in five files and only three of them are generated** —
  ...
  `packaging/mcp/installer/install.py` are regenerated by a command each;
  `packaging/mcp/mcpb/uv.lock` records it too and is refreshed by `uv lock` — the one v0.19.0
  missed, found 2026-09-16 by M144 and now caught by a test; and
  `packaging/mcp/mcpb/manifest.json` is **hand-edited**, because `build_mcpb.py` sets its version
  inside `stage()` — which writes the staged copy under `dist/`, not the committed input.
  M136's runbook line said `--validate` covered the manifest. It does not, and that was corrected
  in the same commit. **The decision is whether to consolidate**, and it is not free either way: one
  `sync_version.py` would end the hand edit and the ordering it implies, but it also becomes a
  fourth generator to keep in step with `sync_pins.py` and `sync_installer.py`, all three doing
  nearly the same thing to different files. A cheaper shape, if consolidation is wanted: teach
  `build_mcpb.py` a `--sync-version` flag, since it already owns the manifest. What makes this
