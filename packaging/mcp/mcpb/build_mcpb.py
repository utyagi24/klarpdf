...
Run it from anywhere::
    ...
    python packaging/mcp/mcpb/build_mcpb.py --validate # manifest + generated pyproject only, no pack
...
ROOT = HERE.parent.parent.parent  # packaging/mcp/mcpb/ -> repo root
...
def render_pyproject(version: str, pins: list[str]) -> str:
    ...
    return f'''# GENERATED by packaging/mcp/mcpb/build_mcpb.py from requirements-mcp.txt — do not hand-edit.
#
...
#
# `requires-python` is the window the ROOT pyproject.toml states and manifest.json restates in
# node-semver; all three are kept in step by tests/test_mcp_packaging.py. See the root file for what
# sets each bound (M132) — floor 3.11 from rpds-py, ceiling 3.15 chosen so the pinned C and Rust
# packages always have a wheel.
[project]
name = "klarpdf-mcp"
version = "{version}"
